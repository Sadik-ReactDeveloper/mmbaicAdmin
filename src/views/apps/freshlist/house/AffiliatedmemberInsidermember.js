...
class AffiliatedmemberList extends React.Component {
  ...
  async componentDidMount() {
    ...
    await this.Alllist(id);
    let CorrectName = Name?.split("_").join(" ");
    let uppercase = CorrectName.charAt(0).toUpperCase() + CorrectName.slice(1);
    this.setState({ username: uppercase });
    let pageparmission = JSON.parse(localStorage.getItem("userData"));
    let newparmisson = pageparmission?.role?.find(
      (value) => value?.pageName === "Affiliated Member List"
    );
              ...
              <Row className="m-2">
                <Col lg="7" sm="7" md="7">
                  <h1 className="float-left">
                    {this.state.username && this.state.username} Member List
                  </h1>
                </Col>
