...
import { Trash2, ChevronDown, Edit, Edit2, Eye } from "react-feather";
...
class ProductType extends React.Component {
                ...
                <Route
                  render={({ history }) => (
                    <Eye
                      ...
                      onClick={() =>
                        history.push(
                          `/app/freshlist/house/inventory/${params?.data?.id}`
                        )
                      }
                    />
                  )}
  ...
  async componentDidMount() {
    ...
    if (pageparmission?.Userinfo?.role == 7) {
      this.props.history.push("/app/freshlist/house/inventory/0");
    } else {
      const formdata = new FormData();
      formdata.append("user_id", pageparmission?.Userinfo?.id);
      formdata.append("role", pageparmission?.Userinfo?.role);
      await axiosConfig.post("/getBranchlist", formdata).then((response) => {
